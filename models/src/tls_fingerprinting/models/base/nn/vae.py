...
class VAE(nn.Module):
    """
    .. inheritance-diagram:: tls_fingerprinting.plugins.core.models.base.nn.vae.VAE
        :parts: 1
    ...
    def generate_noise(self, count: int, n_units: int) -> torch.Tensor:
        mean = torch.zeros(count, n_units)
        std = torch.ones(count, n_units)
        return torch.normal(mean=mean, std=std).to(self.device)
    ...
    def generate(self, count: int, cond: Optional[np.ndarray] = None) -> np.ndarray:
        ...
        for idx in range(steps):
            noise = self.generate_noise(self.batch_size, self.n_units_embedding)
    ...
    def encode(
        ...
        return self._reparameterize(mu, logvar), mu, logvar

    def decode(
        self, noise: torch.Tensor, cond: Optional[torch.Tensor] = None
    ) -> torch.Tensor:
        return self.decoder(noise, cond)
