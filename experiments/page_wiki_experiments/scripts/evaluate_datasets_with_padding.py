...
def evaluate_by_domain_by_horizon(label: str, X: np.ndarray, y: np.ndarray) -> None:
    ...
    for domain in tqdm(common_domains):
        ...
        lho = 5
        rho = 15
        for padblock in [16, 32, 64, 100, 200, 500, 1000, 5000, 10000]:
            other_lim = 60000
            horizon_data = X.copy()
            horizon_labels = pd.Series(y).copy()
            horizon_labels[horizon_labels != domain] = "other"
            ...
            horizon_data[:, lho:rho, :] = vfunc(horizon_data[:, lho:rho, :], padblock)
...
common_domains = pd.Series(y).value_counts().index.values
...
# evaluate full
evaluate_by_domain_by_horizon("advanced", data_temporal, y)
