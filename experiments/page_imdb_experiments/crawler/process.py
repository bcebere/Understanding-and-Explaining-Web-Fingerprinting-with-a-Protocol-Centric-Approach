...
def get_static_stats_per_flow(country: str) -> None:
    workspace = Path(f"traces_{country}")

    output = Path(f"datasets_static_{country}")
    ...
    for filename in files:
        ...
        output_csv = output / f"flow_{stem}.csv"

        if output_csv.exists():
            print("already cached", output_csv, flush=True)
            continue
        ...
        static_stats["request_path"] = meta["request"]["path"]
        static_stats["use_adblocking"] = meta["use_adblocking"]
        static_stats["use_tracking_blocking"] = meta["use_tracking_blocking"]
        static_stats["use_user_cookies"] = meta["use_user_cookies"]
        static_stats["use_http_caching"] = meta["use_http_caching"]
        static_stats["country"] = meta["country"]
        static_stats["network_type"] = meta["network_type"]
        static_stats["meta_file"] = str(file_meta)
        static_stats.to_csv(output_csv, index=False)
...
# get_temporal_stats_per_flow(country = "RO")
